...
router.get('/me', auth,async(req,res)=>{
    try {
        ...
        if(!profile){
            return res.status(400).json({msg:'there is no profile for thise user'});
        }
        res.json(profile);
    } catch (error) {
...
router.post('/',[ auth, [
    ...
    const {company,website,
           location,bio,status,githubusername,skills,
...
// for display suer detail with profile
...
//for inserting experience in profile table
    ...
    try {
        const options={
            uri:`https://api.github.com/users/${
                req.params.username}/repos?per_page=2&sort=created:asc&client_id=${
